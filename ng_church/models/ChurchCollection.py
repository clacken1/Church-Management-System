...
class DonationLine(models.Model):
    ...
    def _prepare_account_voucher(self):
        ...
        payload = {
            ...
            'line_ids': [(0, 0, {#debit
                'name': self.notes,
                'account_id': company.transit_account.id,  # debit account
                'debit': self.amount,
            }),
            (0, 0, { #credit
                'name': self.notes,
...
class TitheLine(models.Model):
    ...
    def generate_tithe_voucher(self):
        """User Interface button call this method."""
        voucher = self.env['account.move']
        voucher_vals = self._prepare_account_voucher()
        ...
        payload = {
            ...
            'line_ids': [
                ...
                (0, 0, {
                    'name': "Offering",
                    'credit': self.amount,
                    'account_id': company.offering_account.id,
                })
            ]
        }
        voucher = voucher.create(payload)
